...
import (
	...
	"github.com/gsmcwhirter/fyne/v2/cmd/fyne/internal/mobile/binres"
	"github.com/gsmcwhirter/fyne/v2/cmd/fyne/internal/templates"
	"github.com/gsmcwhirter/fyne/v2/cmd/fyne/internal/util"
...
func buildAPK(out io.Writer, nmpkgs map[string]map[string]bool, libFiles []string, androidArchs []string) (*Writer, error) {
	...
	for _, arch := range androidArchs {
		toolchain := ndk.Toolchain(arch)
		if nmpkgs[arch]["github.com/gsmcwhirter/fyne/v2/internal/driver/mobile/exp/audio/al"] {
			dst := "lib/" + toolchain.abi + "/libopenal.so"
			src := filepath.Join(gomobilepath, dst)
			if _, err := os.Stat(src); err != nil {
				return nil, errors.New("the Android requires the github.com/gsmcwhirter/fyne/v2/internal/driver/mobile/exp/audio/al, but the OpenAL libraries was not found. Please run gomobile init with the -openal Flag pointing to an OpenAL source directory")
			}
