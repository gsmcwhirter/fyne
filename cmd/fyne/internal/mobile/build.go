...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/gsmcwhirter/fyne/v2/internal/driver/mobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/gsmcwhirter/fyne/v2/internal/driver/mobile/app"`, pkg.PkgPath)
	}

	return pkg, nil
}

var nmRE = regexp.MustCompile(`[0-9a-f]{8} t _?(?:.*/vendor/)?(github.com/gsmcwhirter/fyne/v2/internal/driver/mobile.*/[^.]*)`)

func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/gsmcwhirter/fyne/v2/internal/driver/mobile/app": true}, nil
	}
	r, w := io.Pipe()
